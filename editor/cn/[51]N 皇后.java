 ...
 *//*
            ...
            solveNQueens(queens, n, row + 1, columns | position,
...
class Solution_51 {
    ...
    private static void process(int i, int[] records, int n) {
        if (i == n) {
            return;
        }
        for (int j = 0; j < n; j++) {
    ...
    private static boolean isvalid(int[] records, int i, int j) {
        for (int k = 0; k < i; k++) {
            if (k == j || Math.abs(j - records[k]) == Math.abs(i - k)) {
                return false;
            }
        }
        return true;
